...
\subsection*{\rqi}
\subsubsection*{Motivation} Previous work showed that \rabe{is it SATD or general one} technical debt is widespread, unavoidable, and has a negative impact on the quality of software projects~\cite{Lim2012Software}. Therefore, a priori we expect that removing technical debt is a concern for developers. To understand how developers deal with  \rabe{is it SATD or general one} technical debt we must first quantify how much debt is removed. 
...
\subsubsection*{Approach} To answer this question we \rabe{identify the authors' identity} analyzed the authors of the changes (\textit{i.e.,} commits from the source code repository) that introduced or removed \SATD comments. In order to do that, we first determine the commit in which a \SATD comment was added, then we check the further file versions to determine if there is any commit that removed the \SATD comment. Finally, we compare the authors of the commits to see if they are the same or not. 
...
\conclusion{\rabe{ I think this is a short conclusion } Majority of \SATD is self-removed.}
...
\rabe{we should move this to approach - Survival plot is a technique originating from the medical domain indicating the probability of a patient to survive at least for $x$ days. To estimate this probability one would ideally like to 

...
The main goal of our study is to understand the removal of \SATD. 
\revised{As this is, to the best of our knowledge, the first study of SATD removal, our work is exploratory in nature.}{R3C5} Therefore, we conduct a case study and triangulate our findings by means of a survey
as recommended in the empirical software engineering literature~\cite{Easterbrook2008,RunesonH09}.
We postpone the discussion of the survey to Section~\ref{sec:case_study_results}.

We checkout all versions of five large, well-commented, open source projects. Then, we use the technique recently presented by Maldonado \emph{et al.}~\cite{Maldonado2015TSE} to detect \SATD based on source code comments. Once \SATD has been identified, we can also conclude when and by whom has it been introduced and removed.
%Finally, we use the generated data to perform our case study. 
...
\begin{figure}[tb!]
	...
	\caption{\rabe{Automatically classified data approach overview}}
...
We select projects to cover different application domains, sizes and numbers of contributors. Furthermore, since the \SATD identification heavily depends on source code comments, we selected \emph{well-commented} projects, and since we are interested in changes in \SATD (introduction and removal) \rabe{we have focused on \emph{highly active} projects}. 
All projects are developed in Java and use Git. \rabe{We selected five open source projects namely Camel, Gerrit, Hadoop, Log4j, and Tomcat, and we started analyzing the selected projects on May 1st 2016.}
...
%\alexander{Isn't the following discussion redundant? Why is the syntax of the Java comments important? If possible I would prefer to remove it...} Line, Block and Javadoc comments are distinguished by their syntax and their location in the Java class. For example, Line comments are written with ``//'' followed by the desired comment. Block comments and Javadoc comments have similar syntax. To open a Block comment ``/\text{*}'' are used, whereas Javadoc comments are opened with ``/\text{*}\text{*}''. Both, Block and Javadoc comments, are closed by ``\text{*}/''. In addition, Javadoc comments are written exclusively before methods and constructors, whereas Block comments can be found scattered in the class.  
...
Since we focus on introduction and removal of \SATD historical information about the project files is sought. First, we identify all Java source code files currently available in the latest version of the project. Then, we analyze the source code repository to track all changes done to each file. Each change made to a file produces a different version of that file, and by extracting them we can analyze each file version looking for \rabe{source comments that indicate} \SATD. Git is capable of tracking renamed or moved files based on a similarity threshold~\cite{BirdMSR2009,HataIWPSE-EVOL2011}. In our study, we use this similarity threshold at 90\%, \emph{i.e.}, if a file is renamed or moved to another folder, and is at least 90\% similar to an older version, Git will consider that the file was just moved or renamed. However, if after the change the file is less than 90\% similar with the previous version Git will consider that the original file was removed and a new one was created instead. Whitespace or blank lines are not taken into consideration to calculate the similarity of the files. Using this Git feature, we extract all versions of the Java files that are currently in the repository.  \revised{In this case we consider the first available file version that contains the \SATD as the file version that introduced the \SATD.}
	
\everton{Our tool differentiates renamed and moved files by analyzing git log with the --follow option (i.e., git log --follow --stat=350 --stat-graph-width=2). This command outputs the old and the current path of a moved or renamed file, then we parse this output to do our analysis. Only then we search for deleted files using  a git log diff to summarize only deleted files (i.e., git log --diff-filter=D --summary). Using the git log diff output we check if the file marked as deleted was already identified as a move/rename action.}
	
	
	
\revised{In addition, our analysis is done by comparing all versions of each source code file in the repository. A version of a file is generated whenever a change (i.e., commit) is made. Each different version of a file is then compared and analyzed to present our results. Lastly, the date of the commit that removes the file is considered as the \SATD removal date.}{R3C9} 

% \alexander{shouldn't Latin be in italics?}
The second step to checkout all versions of files is to identify the files that are no longer present in the repository  (\textit{i.e.,} deleted files). Using Git we obtain the list of hashes of commits that has deleted at least one file and the fully qualified names of these files. Using this information we repeat the process described above to obtain all the older versions of the files. To guarantee the correctness of the process we focus solely on Java source files, and ensure that every fully qualified path of the file is analyzed only once. 
...
\rabe{I changed the format of this section to match Figure~\ref{fig:automatically_classified_data_approach_overview}}
We parse the source code in order to extract the source code comments. We use an open source library SrcML~\cite{Collard2013SIE} to parse the source code, and extract the comments and the information related to them such as the line that each comment starts, finishes and the type of the comment (\textit{i.e.,} Javadoc, Line or Block). 
\rabe{is this  done for the current version of the project or all versions?}
%All the extracted information is stored in a relational database to facilitate the processing of the data. 
%
%Once the parsing step is complete, we have at our disposal all of the comments in all versions of the files for each of the five projects. 


%
%\subsection{Using NLP to Identify Self-Admitted Technical Debt}
%
%The state-of-the-art technique to identify \SATD uses Natural Language Processing (NLP) to classify comments based on their contents. 
%Before classifying comments, one needs to remove comments that are irrelevant. 

%\subsubsection{Filtering Irrelevant Comments.} 
As the prior work showed, not all comments can contain \SATD \cite{Potdar2014ICSME,Wehaibi2016SANER}. Therefore,  as the pre-processing we remove the following types of comments from our dataset: %before applying the NLP-based classification to determine \SATD comments:
...
\item \textbf{Commented source code} is also ignored since prior work showed that it generally does not contain \SATD \cite{Potdar2014ICSME,Wehaibi2016SANER}.
...
\subsection{Applying the NLP to Identify \SATD} 
...
\revised{Now, we have a trained NLP classifier (\textit{maximum entropy model}), we follow a general process of machine learning where we next applied the trained NLP model to classify the extracted comments from our five studied projects. We are confident of using the trained NLP classifier which is trained on the data provided in~\cite{Maldonado2015TSE}, since Maldonado \emph{et al.} show that the trained NLP classifier are able to produce good results even when it is evaluated on different data. In their experiment, they trained the NLP classifier on 9 out of 10 projects and evaluated on the remaining one project. They were able to achieve a high result of an average F1-measure of 0.620 when identifying design \SATD, and an average F1-measure of 0.403 when identifying requirement \SATD. Apparently, however, this could affect the results of our study and we   elaborate more on such a threat in Section~\ref{sec:threats_to_validity}. The last two columns in Table~\ref{tab:project_details} show the number of identified \SATD comments in each project after applying the trained NLP classifier.}{R3C11}

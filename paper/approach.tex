...
Since this is, to the best of our knowledge, one of the first studies about the removal of SATD, our work is exploratory in nature. Therefore, we conduct a case study and triangulate our findings by means of a survey
...
All projects are developed in Java and use Git. We selected five open source projects namely Camel, Gerrit, Hadoop, Log4j, and Tomcat, and we started analyzing the selected projects on May 1st 2016.
...
Since we focus on introduction and removal of \SATD historical information about the project files is sought. First, we identify all Java source code files currently available in the latest version of the project. Then, we analyze the source code repository to track all changes done to each file. Each change made to a file produces a different version of that file, and by extracting them we can analyze each file version looking for source comments that indicate \SATD. Git is capable of tracking renamed or moved files based on a similarity threshold~\cite{BirdMSR2009,HataIWPSE-EVOL2011}. In our study, we use this similarity threshold at 90\%, \emph{i.e.}, if a file is renamed or moved to another folder, and is at least 90\% similar to an older version, Git will consider that the file was just moved or renamed. However, if after the change the file is less than 90\% similar with the previous version Git will consider that the original file was removed and a new one was created instead. Whitespace or blank lines are not taken into consideration to calculate the similarity of the files. Using this Git feature, we extract all versions of the Java files that are currently in the repository.  In this case we consider the first available file version that contains the \SATD as the file version that introduced the \SATD. Our tool differentiates renamed and moved files by analyzing git log with the --follow option (i.e., git log --follow --stat=350 --stat-graph-width=2). This command outputs the old and the current path of a moved or renamed file, which we then parse to perform our analysis. Only then we search for deleted files using  a git log diff to summarize only deleted files (i.e., git log --diff-filter=D --summary). Using the git log diff output we check if the file marked as deleted was already identified as a move/rename action.
	
	
	
In addition, our analysis is done by comparing all versions of each source code file in the repository. A version of a file is generated whenever a change (i.e., commit) is made. Each different version of a file is then compared and analyzed to present our results. Lastly, the date of the commit that removes the file is considered as the \SATD removal date.

% \alexander{shouldn't Latin be in italics?}
The second step to checkout all versions of files is to identify the files that are no longer present in the repository  (\textit{i.e.,} deleted files). Using Git we obtain the list of commit hashes that have deleted at least one file and the fully qualified names of these files. Using this information we repeat the process described above to obtain all the older versions of the files. To guarantee the correctness of the process we focus solely on Java source files, and ensure that every fully qualified path of the file is analyzed only once. 

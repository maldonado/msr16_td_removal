...
% First, we group the different fully qualified names that we obtained for each file. For example, if ``file1'' was in ``path1'' during 10 versions and then it was moved to ``path2''. The group of fully qualified names for ``file1'' would have two items: ``path1/file1'' and ``path2/file1''. Then, we extract the list of changes done 

% As we identified all fully qualified names of each file in the previous step we can use this information to get all commits that changed the file 
% First, we identifyall the commits that touched that particular file. Then, we use the commit hash place the repository in the time that the change was made. As we identi

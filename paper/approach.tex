...
As this is, to the best of our knowledge, the first study of SATD removal, our work is exploratory in its nature. Therefore, we conduct a case study and triangulate our findings by means of a survey
...
\subsection{Identifying \SATD comments}
\label{sub:parse_source_code}

We parse the source code in order to extract the comments. We use an open source library SrcML~\cite{Collard2013SIE} to parse the source code, and extract the comments and the information related to them such as the line that each comment starts, finishes and the type of the comment (\textit{i.e.,} Javadoc, Line or Block). 
...
Next, we use the technique presented in by Maldonado \emph{et al.}~\cite{Maldonado2015TSE} to identify the \SATD comments. We refer readers to Maldonado \emph{et al.}'s paper for full details on how to identify \SATD comments, however, to make our paper self-contained, we highlight the key points of their approach next.
%
...
\subsubsection{Applying the NLP Classifier.} 
We use the Stanford NLP Classifier~\cite{Manning2014ACL} to classify \SATD comments.

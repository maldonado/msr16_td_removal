...
\revised{As this is, to the best of our knowledge, the first study of SATD removal,}{R3C5} our work is exploratory in nature. Therefore, we conduct a case study and triangulate our findings by means of a survey
...
We start by selecting case study projects. 
While sometimes even a single case might be sufficient, \emph{e.g.}, when it is typical, we study several
systems as multiple case design is known to usually offer greater validity~\cite{Easterbrook2008}.
We select projects to cover different application domains, system sizes and numbers of contributors. Furthermore, since the \SATD identification heavily depends on source code comments, we selected \emph{well-commented} projects, and since we are interested in changes in \SATD (introduction and removal) we have focused on \emph{highly active} projects. 
All projects are developed in Java and use Git.
...
Since we focus on introduction and removal of \SATD historical information about the project files is sought. First, we identify all Java source code files currently available in the latest version of the project. Then, we analyze the source code repository to track all changes done to each file. Each change made to a file produces a different version of that file, and by extracting them we can analyze each file version looking for \SATD. Git is capable of tracking renamed or moved files based on a similarity threshold~\cite{BirdMSR2009,HataIWPSE-EVOL2011}. In our study, we use this similarity threshold at 90\%, \emph{i.e.}, if a file is renamed or moved to another folder, and is at least 90\% similar to an older version, Git will consider that the file was just moved or renamed. However, if after the change the file is less than 90\% similar with the previous version Git will consider that the original file was removed and a new one was created instead. \revised{Also, we consider deleted file as}{R3C9} Whitespace or blank lines are not taken into consideration to calculate the similarity of the files. Using this Git feature, we extract all versions of the Java files that are currently in the repository. 
...
\revised{It is worth mentioning that we consider all files that exist in the analyzed version of the project and we do not consider files that existed and deleted through the history of the project.}{R3C9}
...
\rabe{This is done fro the current version of the project?}
...
\begin{itemize}
	\item To train the NLP classifier, we used the manually classified \SATD comments data set provided by Maldonado \emph{et al.}~\cite{Maldonado2015TSE}. The data set contains contains 62,566 comments extracted from ten open source projects. These comments were classified as \SATD comments or as regular comments (\textit{i.e.,} comments without \SATD). The manually classified data set was verified by the authors of the paper and they showed that two independent reviewers agreed on the classification, achieving a Cohen's Kappa value of +0.81. 
...
\revised{Now, we have trained NLP classifier (\textit{maximum entropy model}), we next applied the trained NLP model to classified the extracted comments from our five studied projects. The last two columns in Table~\ref{tab:project_details} show the number of identified \SATD comments in each project.}{R3C11}

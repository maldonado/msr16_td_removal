...
To answer the aforementioned questions, we leverage a natural language processing (NLP) based technique to determine SATD introduction and removal. In total, we examine 5,733 SATD removals in five large open source systems. Our findings indicate that 1) the majority of \SATD comments are removed and in the studied projects the removal ranges between 40.5--90.6\%, and on average 74.9\% of the identified \SATD is removed; 2) most \SATD (on average 54.4\% and median 61.0\%) is self-removed; 3) the median amount of time that \SATD stays in the project ranged between 82-613.2 days on average and 18.2-172.8 days; and 4) developer add \SATD to track potential future bugs and code that needs improvements, whereas, developers mostly remove \SATD when they are fixing bugs or adding new features. Very seldom do developers remove \SATD as part of refactoring efforts or dedicated code improvement activities.

Our empirical findings provide insights to developers and software projects \rabe{managers} on how to best manage \SATD. For example, our finding showing that most \SATD is self-removed provides insights on who typically removes \SATD in large open source projects.
